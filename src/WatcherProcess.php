...
class WatcherProcess extends Process {
    ...
    protected function recommendAssertionSetting() {
        if (ini_get("zend.assertions") === "1") {
            $this->logger->warning(
                "Running a cluster in production with assertions enabled is not recommended; " .
        ...
        if (\extension_loaded("xdebug")) {
            $this->logger->warning(
                "Running a cluster in production with the xdebug extension loaded is not recommended; " .
    ...
    protected function recommendLogLevel(Console $console) {
        ...
        if (Logger::LEVELS[$level] === Logger::LEVELS[Logger::DEBUG]) {
            $this->logger->debug(
                "Running a cluster in production with a log level equal to \"debug\" is not " .
